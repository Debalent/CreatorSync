...
let beatMakerSynth = null;
let beatMakerAnalyzer = null;
...
async function initializeBeatMaker () {
  try {
    ...
    beatMakerSynth = new BeatMakerSynth(beatMaker, beatMakerUI); // eslint-disable-line no-undef
    beatMakerSynth.initialize();
    beatMakerSynth.setupListeners();
    beatMakerSynth.mountKeyboard('synthMiniKeyboard');

    // Initialize spectrum analyzer
    beatMakerAnalyzer = new BeatMakerAnalyzer(beatMaker); // eslint-disable-line no-undef
    beatMakerAnalyzer.initialize();
    beatMakerAnalyzer.setupListeners();
    const analyzerCanvas = document.getElementById('analyzerCanvas');
    if (analyzerCanvas) beatMakerAnalyzer.mount(analyzerCanvas);
...
window.addEventListener('beforeunload', () => {
  beatMakerSynth?.dispose();
  beatMakerAnalyzer?.dispose();
  if (beatMaker) {
    beatMaker.dispose();
  }
});
