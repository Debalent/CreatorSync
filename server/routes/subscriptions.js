...
router.get('/finisher-access', authenticateUser, async (req, res) => {
    try {
        const userId = req.user.id || req.user.userId;

        // Get user from data manager
        const dataManager = require('../utils/dataManager');
        const user = await dataManager.findUserById(userId);
        ...
        if (hasActiveSubscription) {
            ...
            res.json({
                ...
                features: tierFeatures[subscription.tier] || tierFeatures.Pro,
                finisherUrl: `/finisher-app.html`
            });
        } else {
            res.json({
                success: true,
