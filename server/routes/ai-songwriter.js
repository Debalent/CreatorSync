...
// In-memory storage for user writing patterns
const userPatterns = new Map();
const writingHistory = new Map();

// ─── PERSISTENCE ─────────────────────────────────────────────────────────────

const PROFILES_FILE = path.join(__dirname, '../../data/ai-songwriter-profiles.json');
...
function loadProfiles () {
    try {
        if (fs.existsSync(PROFILES_FILE)) {
            ...
            if (data.userPatterns) Object.entries(data.userPatterns).forEach(([k, v]) => userPatterns.set(k, v));
            if (data.writingHistory) Object.entries(data.writingHistory).forEach(([k, v]) => writingHistory.set(k, v));
            console.log(`[AI Songwriter] Loaded ${userPatterns.size} user profiles from disk`);
        }
    } catch (e) {
        console.error('[AI Songwriter] Failed to load profiles:', e.message);
    }
}
...
function saveProfiles () {
    try {
        const data = {
            ...
            writingHistory: Object.fromEntries(writingHistory)
        };
        fs.writeFileSync(PROFILES_FILE, JSON.stringify(data));
    } catch (e) {
        console.error('[AI Songwriter] Failed to save profiles:', e.message);
    }
}
...
/**
 * Get full user style profile (for loading on page init)
 */
router.get('/profile/:userId', (req, res) => {
    try {
        ...
        if (!patterns) {
            return res.json({ success: true, profile: null, songsAnalyzed: 0, learningProgress: 0 });
        }

        const avg = (arr) => Array.isArray(arr) && arr.length > 0
            ? arr.reduce((a, b) => a + (typeof b === 'number' ? b : 0), 0) / arr.length
            : 50;

        // Most-used rhyme scheme
        const schemeFreq = {};
        (patterns.rhymeScheme || []).forEach(s => { schemeFreq[s] = (schemeFreq[s] || 0) + 1; });
        const preferredScheme = Object.entries(schemeFreq).sort((a, b) => b[1] - a[1])[0]?.[0] || '';

        // Dominant emotion across history
        const emotionFreq = {};
        (patterns.emotion || []).forEach(e => { emotionFreq[e] = (emotionFreq[e] || 0) + 1; });
        ...
        res.json({
            ...
            profile: {
                ...
                preferredRhymeScheme: preferredScheme,
                personalVocab: patterns.personalVocab || {},
                appliedSuggestions: patterns.appliedSuggestions || { line: 0, word: 0, structure: 0 }
            },
            songsAnalyzed: Math.floor((patterns.cadence || []).length / 2),
...
/**
 * Record which suggestion type the user applied (feedback loop)
 */
router.post('/feedback', (req, res) => {
    try {
        const { userId, suggestionType } = req.body;
        if (!userId || !suggestionType) {
            return res.status(400).json({ error: 'userId and suggestionType are required' });
        }

        const patterns = userPatterns.get(userId);
        if (patterns) {
            if (!patterns.appliedSuggestions) patterns.appliedSuggestions = { line: 0, word: 0, structure: 0 };
            patterns.appliedSuggestions[suggestionType] = (patterns.appliedSuggestions[suggestionType] || 0) + 1;
            saveProfiles();
        }
...
router.post('/analyze-style', async (req, res) => {
    try {
        ...
        analysis.rawLyrics = lyrics; // attach raw for vocab extraction
...
function updateUserPatterns (userId, analysis) {
    if (!userPatterns.has(userId)) {
        userPatterns.set(userId, {
            ...
            rhymeScheme: [],
            personalVocab: {},
            appliedSuggestions: { line: 0, word: 0, structure: 0 }
        });
    }
    ...
    // Build personal vocabulary frequency map from raw lyrics
    if (analysis.rawLyrics) {
        ...
        if (!patterns.personalVocab) patterns.personalVocab = {};
        const words = analysis.rawLyrics.toLowerCase().split(/\s+/);
        words.forEach(w => {
            const clean = w.replace(/[^a-z]/g, '');
            if (clean.length > 3 && !STOP_WORDS.has(clean)) {
                patterns.personalVocab[clean] = (patterns.personalVocab[clean] || 0) + 1;
            }
        });
        // Keep top 100 words only
        const sorted = Object.entries(patterns.personalVocab).sort((a, b) => b[1] - a[1]).slice(0, 100);
        patterns.personalVocab = Object.fromEntries(sorted);
    }

    // Keep only last 20 entries per metric for performance
    ['cadence', 'tone', 'rhythm', 'vocabulary', 'emotion', 'lineLength', 'rhymeScheme'].forEach(key => {
        if (patterns[key].length > 20) patterns[key] = patterns[key].slice(-20);
    });
