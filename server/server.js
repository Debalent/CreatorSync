...
class CreatorSyncServer {
    constructor () {
        ...
        const socketOrigins = (process.env.CLIENT_URL || 'http://localhost:5501')
            .split(',')
            .map(o => o.trim())
            .filter(Boolean);
    ...
    initializeMiddleware () {
        ...
        // CORS configuration — supports comma-separated CLIENT_URLS for multi-origin (local + CloudFront + custom domain)
        const allowedOrigins = (process.env.CLIENT_URL || 'http://localhost:5501')
            .split(',')
            .map(o => o.trim())
            .filter(Boolean);
